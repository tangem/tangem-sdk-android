...
class SdkProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        try {
            ...
            angleToSwitchSweepIncrement = attributes.getInt(R.styleable.SdkProgressBar_pb_sweep_start_increment, angleToSwitchSweepIncrement)
            angleToSwitchSweepDecrement = attributes.getInt(R.styleable.SdkProgressBar_pb_sweep_start_decrement, angleToSwitchSweepDecrement)
            progressBarColor = attributes.getInt(R.styleable.SdkProgressBar_pb_color, progressBarColor)
            progressBarThickness = attributes.getDimension(R.styleable.SdkProgressBar_pb_thickness, progressBarThickness).pxToDp()
            secondaryProgressBarColor = attributes.getInt(R.styleable.SdkProgressBar_pb_secondary_color, secondaryProgressBarColor)
            secondaryProgressBarThickness = attributes.getDimension(R.styleable.SdkProgressBar_pb_secondary_thickness, secondaryProgressBarThickness).pxToDp()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isIndeterminate) {
            val step = if (sweepState == SweepState.INCREMENT) sweepStep
            else sweepStep * 2
            startAngle = (startAngle + step) % 360f

            if (sweepState == SweepState.INCREMENT) sweepAngle += sweepStep
            else sweepAngle -= sweepStep
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val highStroke = if (progressBarThickness > secondaryProgressBarThickness) progressBarThickness else secondaryProgressBarThickness
